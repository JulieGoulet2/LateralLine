...
**Quantitative analysis (2026-07-07) — the bands are feedforward and reflect bipolar dipole geometry.** A feedforward reconstruction of each TS tuning curve (LL forward model → trained LL→MON weights → trained MON→TS weights, with a per-layer global threshold standing in for spiking + global inhibition, no Brian2 simulation, plasticity-free; `plots/tuning_multimodality.py`) reproduces the multimodality directly from the trained weights. Across the 6 topo = 0.20 seeds (127–132), 41 % of active TS cells have ≥ 2 peaks (mean 1.48 peaks/cell, up to 4), matching the 2–3 distinct positions seen in the spike rasters. Because the reconstruction is feedforward and plasticity-free, the multimodality cannot be a spiking- or learning-specific artifact — it is inherited from the trained weight structure and the stimulus geometry. The LL representation itself explains the origin: the correlation matrix C[x, x′] = corr(r_LL(x), r_LL(x′)) over the 100 neuromasts is **bipolar** — LL patterns decorrelate within ~0.6 cm and then become strongly *anti*-correlated (min corr ≈ −0.48 at ~1.1 cm separation, the signature of the sign-changing dipole velocity field), with only a weak positive re-correlation (max ≈ +0.36) at large separation. This **refines** the interpretation above: distant x positions do **not** produce *identical* LL patterns (no distant correlation reaches r ≥ 0.8), but the bipolar structure of the field re-drives a given TS cell at a second, well-separated x (secondary peaks appear at a characteristic ~1.8–2.3 cm offset). The conclusion is unchanged and strengthened: the multimodality is an intrinsic property of lateral-line dipole geometry, and the somatotopic map is a population-level phenomenon. *Caveat: the rate-based reconstruction sharpens tuning but does not reproduce the full-height population stripes seen in the raster, which additionally require spiking synchrony.*

![](Picture/tuning_multimodality.png)

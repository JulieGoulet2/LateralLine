...
def make_ll_snapshots(n_trials, trial_duration_s, dt_s, n_ll, seed, distance_cm=0.8, ll_rate_gain=2.0):
    rng = np.random.default_rng(seed)
    ...
    ll_rates_arr = np.zeros((n_steps, n_ll), dtype=float)
    ...
    while idx < n_steps:
        ...
        snap = np.clip(snap - stim_params.r0_hz, 0.0, None) * float(ll_rate_gain)
        j0 = idx
        j1 = min(n_steps, idx + int(trial_duration_s / dt_s))
        ll_rates_arr[j0:j1, :] = snap[None, :]
        x_trace[j0:j1] = x_cm
        idx = j1
        t += 1

    return ll_rates_arr, x_trace
...
def run_ll_mon_learning(
    ...
    # Stronger MON background drive for the design script so that
    # spiking is robust even without TS or additional circuitry.
    bg_rate_mon_hz=300.0,
    bg_w_mon_mV=6.0,
    mon_global_inh_mV=0.0,
    ll_rate_gain=2.0,
    seed=123,
):
    # Build LL snapshots (avoid name 'rates' for Brian2 namespace).
    ll_rates_arr, x_trace = make_ll_snapshots(
        n_trials=n_trials,
    ...
    ll_rates_ta = b2.TimedArray(ll_rates_arr * b2.Hz, dt=dt_s * b2.second)
    ll = b2.PoissonGroup(n_ll, rates="ll_rates_ta(t, i)", namespace={"ll_rates_ta": ll_rates_ta})
    ...
    ns = {
        "El": -74.0 * b2.mV,
        # Make MON neurons more excitable in the design script so that
        # LL drive + background reliably elicit spikes.
        "Vth": -60.0 * b2.mV,
        "Vreset": -60.0 * b2.mV,
        "tau_ref": 2.0 * b2.ms,
        "tau_m": 10.0 * b2.ms,
    ...
    # Target incoming sum per MON (use distinct names to avoid Brian2 namespace conflict).
    w_init_mV = np.array(s_ll_mon.w[:] / b2.mV, dtype=float, copy=True)
    jj_conn = np.array(s_ll_mon.j[:], dtype=int, copy=True)
    incoming0 = np.bincount(jj_conn, weights=w_init_mV, minlength=n_mon)
    ...
    for k in range(n_steps):
        ...
        # Homeostatic normalization (use w_ll_arr to avoid Brian2 'w' namespace conflict).
        if ll_mon_homeo_eta > 0.0 and ((k + 1) % max(1, int(ll_mon_homeo_every_trials)) == 0):
            ...
            jj_conn = np.array(s_ll_mon.j[:], dtype=int, copy=True)
            incoming = np.bincount(jj_conn, weights=w_ll_arr, minlength=n_mon)
            ...
            if np.any(nonzero):
                ...
                w_ll_arr = np.clip(w_ll_arr * scale[jj_conn], 0.0, ll_mon_wmax_mV)
...
def plot_results(result: dict, out_dir: Path):
    ...
    for ax, mj in zip(axes, mon_indices):
        ...
        w_before = w0[m]
        w_after = w1[m]
        ax.stem(x, w_before, basefmt=" ", linefmt="C0-", markerfmt="C0o", label="before")
        ax.stem(x, w_after, basefmt=" ", linefmt="C1-", markerfmt="C1^", label="after")
...
def main():
    ...
    rate_mon = result["sp_mon"].num_spikes / max(1, result["n_mon"] * tot_s)
    print(f"MON rate per neuron: {rate_mon:.4f} Hz")
    print(f"Target incoming LL->MON per MON: {result['target_incoming']:.4f} mV")
    print("Finished LL->MON design run. See:", out_dir)
